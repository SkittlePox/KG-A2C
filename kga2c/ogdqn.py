...
from models import KGA2C, KGOA2C
from env import *
from vec_env import *
import logger
...
class KGOA2CTrainer(object):
    ...
    def __init__(self, params):
        ...
        # f = open("info/jericho-grammar.txt", 'w')
        # f.write(str(self.binding['grammar']))
        # f.close()
        ...
        # f = open('info/templates.txt', 'w')
        # f.write(str(self.template_generator.templates))
        # f.close()
        ...
        for trans, ret, adv in zip(transitions, returns, advantages):
            ...
            for d, st in enumerate(dec_steps):
                if st > 1:
                    ...
                    o1_mask[d] = 1
            o1_mask = torch.FloatTensor(o1_mask).to(device)#.cuda()
            o2_mask = torch.FloatTensor(o2_mask).to(device)#.cuda()

            # Policy Gradient Loss
            policy_obj_loss = torch.FloatTensor([0]).to(device)#.cuda()
            cnt = 0
